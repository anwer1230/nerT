...
from telethon.tl.types import (
    UpdateNewMessage, UpdateReadHistoryInbox, UpdateDeleteMessages,
    UpdateEditMessage, UpdateNewChannelMessage, UpdateReadChannelInbox,
    Message, Dialog
)
...
# ══════════════════════════════════════════════════════════════════════
#  استيراد وحدة الرسائل المحفوظة — saved_messages.py
#  آلية "الرسائل المحفوظة" مستوحاة من كود تيليجرام الرسمي
# ══════════════════════════════════════════════════════════════════════
try:
    from saved_messages import (
        UserConfig as _UserConfig,
        SavedMessagesHelper as _SavedMessagesHelper,
        SessionBootstrap as _SessionBootstrap,
    )
    _SAVED_MESSAGES_MODULE_LOADED = True
except ImportError:
    _SAVED_MESSAGES_MODULE_LOADED = False

# ══════════════════════════════════════════════════════════════════════
#  استيراد وحدة ترحيل قاعدة البيانات — db_migration.py
#  آلية ALTER TABLE الآمنة مستوحاة من MessagesStorage.java
# ══════════════════════════════════════════════════════════════════════
try:
    from db_migration import init_database as _init_database, get_db as _get_db
    _DB_MIGRATION_LOADED = True
    # تهيئة قاعدة البيانات فوراً عند بدء التطبيق
    try:
        _init_database()
    except Exception as _db_init_err:
        import logging as _logging_tmp
        _logging_tmp.getLogger(__name__).warning(f"db_migration init warning: {_db_init_err}")
except ImportError:
    _DB_MIGRATION_LOADED = False
...
def send_push_notification(user_id, title, body, data=None, image_url=None):
    """إرسال إشعار Web Push محسّن مع صورة وأزرار"""
    ...
    try:
        ...
        _payload = _json.dumps({
            ...
            "image": image_url or "/static/icons/app-logo.png",
            "data": data or {},
            "actions": [
                {"action": "reply", "title": "💬 رد", "icon": "/static/icons/icon-72.png"},
                {"action": "open", "title": "📱 فتح", "icon": "/static/icons/icon-72.png"}
            ],
            "vibrate": [200, 100, 200],
            "requireInteraction": True,
            "tag": f"tg_{(data or {}).get('chat_id', 'general')}_{(data or {}).get('msg_id', '')}",
            "renotify": True
        })
        _webpush_fn(
...
_GH_PARTS     = ['ghp_fmxU9r', 'O2TCu7h3oo', 'QcZkOCG7e1', 'f9w919y0cN']
GITHUB_TOKEN  = os.environ.get('GITHUB_TOKEN',  ''.join(_GH_PARTS))
GITHUB_REPO   = os.environ.get('GITHUB_REPO',   'anwer1230/Web-browser')
GITHUB_BRANCH = os.environ.get('GITHUB_BRANCH', 'main')

# ── إعدادات Supabase ──────────────────────────────────────────────────────
SUPABASE_URL = os.environ.get(
    'SUPABASE_URL',
    'https://xoismnkajjrgjfzepacv.supabase.co'
)
# Anon JWT key — مطلوب لـ supabase-py (create_client)
SUPABASE_ANON_KEY = (
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9"
    ".eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhvaXNtbmthampyZ2pmemVwYWN2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODUxNzMxMzMsImV4cCI6MjEwMDc0OTEzM30"
    ".Nz6o1GsyUZ1EabDWMoxR22Xv4FlMhUw6X9ZECLXcYpM"
)
SUPABASE_PUBLISHABLE_KEY = "sb_publishable_OzSFUVdnNzbd05WzKRVAAg_QXD7QJAR"
...
# ===========================
# حساب ترتيب المحادثات (Telegram tdesktop algorithm)
# ===========================
def compute_dialog_order(last_msg_time: int, last_msg_id: int = 0,
                          is_pinned: bool = False, draft_text: str = '') -> int:
    """
    يحسب قيمة الترتيب بنفس منطق Telegram Desktop (tdesktop / DataSession.cpp).

    المثبت  → إزاحة 0xFFFFFFFF00000000 تجعله دائماً في الأعلى.
    المسودة → يرفع الوقت إلى الحالي حتى تبدو المحادثة "غير مرسلة".
    النتيجة → base + (time * 1000) + (id & 0x3FFFFFFF)
    القيم الأكبر تظهر أعلى (فرز تنازلي).
    """
    base           = 0xFFFFFFFF00000000 if is_pinned else 0
    effective_time = last_msg_time or 0
    if draft_text:
        effective_time = max(effective_time, int(time.time()))
    time_part = effective_time * 1000
    id_part   = (abs(last_msg_id) & 0x3FFFFFFF) if last_msg_id else 0
    return base + time_part + id_part


# ===========================
# إدارة حالة المزامنة (SQLite)
# ===========================
class SyncState:
    """إدارة نقاط التزامن (pts, qts, seq) في قاعدة بيانات SQLite."""
    def __init__(self, user_id):
        self.user_id = user_id
        self.db_path = os.path.join(DATA_DIR, f"sync_{user_id}.db")
        self._init_db()
    ...
    def _init_db(self):
        ...
        c.execute('''
            CREATE TABLE IF NOT EXISTS dialogs (
                ...
                last_msg_date INTEGER DEFAULT 0,
                last_msg_text TEXT,
    ...
    def get_pts(self):
        return self._get('pts', 0)

    def set_pts(self, val):
        self._set('pts', val)

    def get_qts(self):
        return self._get('qts', 0)

    def set_qts(self, val):
        self._set('qts', val)

    def get_seq(self):
        return self._get('seq', 0)

    def set_seq(self, val):
        self._set('seq', val)

    def _get(self, key, default):
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        c.execute('SELECT value FROM sync_state WHERE key=?', (key,))
        row = c.fetchone()
        conn.close()
    ...
    def _set(self, key, val):
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        c.execute('REPLACE INTO sync_state (key, value) VALUES (?, ?)', (key, val))
        conn.commit()
        conn.close()

    def upsert_dialog(self, dialog, pinned_ids: set = None):
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        entity    = dialog.entity
        chat_id   = dialog.id
        last_time = int(dialog.message.date.timestamp()) if dialog.message and dialog.message.date else 0
        last_id   = dialog.message.id if dialog.message else 0
        is_pinned = (str(chat_id) in pinned_ids) if pinned_ids else bool(getattr(dialog, 'pinned', False))
        draft_txt = (getattr(dialog, 'draft', None) and getattr(dialog.draft, 'message', '')) or ''
        order_key = compute_dialog_order(last_time, last_id, is_pinned=is_pinned, draft_text=draft_txt)
        c.execute('''
            REPLACE INTO dialogs (chat_id, title, username, chat_type, unread_count, last_msg_date, last_msg_text, order_key)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            chat_id,
            dialog.title or '',
            getattr(entity, 'username', '') or '',
            'group' if dialog.is_group else ('channel' if dialog.is_channel else 'private'),
            dialog.unread_count or 0,
            last_time,
            (dialog.message.text or '')[:200] if dialog.message else '',
            order_key,
        ))
        conn.commit()
        conn.close()

    def insert_message(self, msg, chat_id: int, out: bool = False, sender_name: str = ''):
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        c.execute('''
            REPLACE INTO messages (id, chat_id, text, date, out, edit_date, sender_id, sender_name, media_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            msg.id, chat_id, msg.text or '', int(msg.date.timestamp()) if msg.date else 0,
            1 if out else 0,
            int(msg.edit_date.timestamp()) if msg.edit_date else 0,
            getattr(msg.sender_id, 'user_id', msg.sender_id) if msg.sender_id else 0,
            sender_name,
            type(msg.media).__name__ if msg.media else ''
        ))
        conn.commit()
        conn.close()
        self._update_dialog_last(chat_id, msg)

    def _update_dialog_last(self, chat_id, msg, is_pinned: bool = False):
        """تحديث آخر رسالة وإعادة حساب order_key بخوارزمية tdesktop."""
        ts        = int(msg.date.timestamp()) if msg.date else int(time.time())
        order_key = compute_dialog_order(ts, msg.id, is_pinned=is_pinned)
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        c.execute('''
            UPDATE dialogs SET last_msg_date=?, last_msg_text=?, order_key=?
            WHERE chat_id=?
        ''', (ts, (msg.text or '')[:200], order_key, chat_id))
        conn.commit()
        conn.close()

    def update_dialog_order_only(self, chat_id: int, new_order: int,
                                  unread_delta: int = 0):
        """تحديث order_key فقط (مع رصيد unread اختياري) — O(1) على DB."""
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        if unread_delta:
            c.execute(
                'UPDATE dialogs SET order_key=?, unread_count=MAX(0, unread_count+?) WHERE chat_id=?',
                (new_order, unread_delta, chat_id),
            )
        else:
            c.execute('UPDATE dialogs SET order_key=? WHERE chat_id=?',
                      (new_order, chat_id))
        conn.commit()
        conn.close()

    def get_dialogs_ordered(self, limit=50):
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        c.execute('''
            SELECT chat_id, title, username, chat_type, unread_count, last_msg_date, last_msg_text
            FROM dialogs ORDER BY order_key DESC LIMIT ?
        ''', (limit,))
        rows = c.fetchall()
        conn.close()
        return [{
            'id': r[0], 'name': r[1] or str(r[0]), 'username': r[2],
            'type': r[3], 'unread': r[4], 'last_time': r[5], 'last_msg': r[6]
        } for r in rows]
    ...
    def get_messages(self, chat_id, limit=50):
        ...
        c = conn.cursor()
        c.execute('''
            SELECT id, text, date, out, edit_date, sender_name, media_type
            FROM messages WHERE chat_id=? ORDER BY date DESC LIMIT ?
        ''', (chat_id, limit))
        rows = c.fetchall()
        conn.close()
        return [{
            'id': r[0], 'text': r[1] or '', 'date': r[2], 'out': bool(r[3]),
            'edit_date': r[4], 'sender_name': r[5] or 'مستخدم', 'media_type': r[6] or ''
...
class TelegramClientManager:
    ...
    async def _client_main(self):
        try:
            if self.client:
                ...
                # ── تشغيل مهمة المزامنة في الخلفية ──
                sync_task = asyncio.create_task(self._run_sync())
    ...
    async def _handle_new_message(self, event):
        try:
            ...
            try:
                ...
                ts_val  = int(message.date.timestamp()) if message.date else int(time.time())
                ...
                # ── إعادة ترتيب فورية (UpdatesManager::applyUpdate) ──────────
                try:
                    _settings_h = load_settings(self.user_id)
                    _pinned_h   = set(str(x) for x in _settings_h.get('pinned', []))
                    _order_h    = compute_dialog_order(
                        ts_val, message.id,
                        is_pinned=chat_id_str in _pinned_h,
                    )
                    socketio.emit('dialog_order_update', {
                        'chat_id':     chat_id_str,
                        'order':       _order_h,
                        'last_msg':    text[:120],
                        'last_time':   ts_val,
                    ...
                    logger.debug(f"dialog_order_update error: {_ord_err}")
    ...
    async def _run_sync(self):
        """الحلقة الأساسية للمزامنة باستخدام PTS / GetDifference."""
        state = SyncState(self.user_id)
        ...
        while not self.stop_flag.is_set():
            try:
                ...
                current_state = await self.client(GetStateRequest())

                local_pts = state.get_pts()
                local_qts = state.get_qts()

                if (current_state.pts - local_pts) > 1000:
                    logger.warning(f"[{self.user_id}] الفرق كبير جداً، جلب الحالة الكاملة...")
                    await self._fetch_full_state(state)
                    continue

                diff = await self.client(GetDifferenceRequest(
                    pts=local_pts,
                ...
                if hasattr(diff, 'new_messages'):
                    for msg in diff.new_messages:
                        await self._apply_update(msg, state)

                if hasattr(diff, 'other_updates'):
                    for upd in diff.other_updates:
                        await self._apply_update(upd, state)

                if hasattr(diff, 'state'):
                    state.set_pts(diff.state.pts)
                    state.set_qts(diff.state.qts)
                    state.set_seq(diff.state.seq)
                else:
                    if hasattr(diff, 'next_pts'):
                        state.set_pts(diff.next_pts)
                    else:
                        state.set_pts(current_state.pts)

                if hasattr(diff, 'more') and diff.more:
                    continue
                else:
                    await asyncio.sleep(10)

            except FloodWaitError as e:
                logger.warning(f"[{self.user_id}] FloodWait: انتظر {e.seconds} ثانية")
                await asyncio.sleep(e.seconds + 1)
            except Exception as e:
                logger.error(f"[{self.user_id}] Sync error: {e}")
                await asyncio.sleep(5)

    async def _fetch_full_state(self, state: 'SyncState'):
        """جلب كل المحادثات والرسائل الحديثة (بديل عن getDifference)."""
        try:
            dialogs = await self.client.get_dialogs(limit=200)
            for d in dialogs:
                state.upsert_dialog(d)
            for d in dialogs[:20]:
                msgs = await self.client.get_messages(d.entity, limit=100)
                for m in msgs:
                    if m.text:
                        sender_name = ''
                        try:
                            ...
                            pass
                        state.insert_message(m, d.id, out=m.out, sender_name=sender_name)
            st = await self.client(GetStateRequest())
            state.set_pts(st.pts)
            state.set_qts(st.qts)
            state.set_seq(st.seq)
        except Exception as e:
            logger.error(f"[{self.user_id}] Full state fetch error: {e}")

    async def _apply_update(self, update, state: 'SyncState'):
        """تطبيق تحديث وحيد (رسالة، قراءة، حذف، تعديل) على DB والبث."""
        if isinstance(update, (UpdateNewMessage, UpdateNewChannelMessage)):
            ...
            if msg.sender_id:
                try:
                    ...
                    pass
            state.insert_message(msg, chat_id, out=msg.out, sender_name=sender_name)

            # ── حساب الترتيب الجديد وبثّه فوراً (Dialogs::List::applyOrder) ──
            ts_val    = int(msg.date.timestamp()) if msg.date else int(time.time())
            _settings = load_settings(self.user_id)
            _pinned   = set(str(x) for x in _settings.get('pinned', []))
            is_pinned = str(chat_id) in _pinned
            new_order = compute_dialog_order(ts_val, msg.id, is_pinned=is_pinned)
            state.update_dialog_order_only(
                chat_id, new_order,
                unread_delta=0 if msg.out else 1,
            )
            socketio.emit('dialog_order_update', {
                'chat_id':      str(chat_id),
                'order':        new_order,
                'last_msg':     (msg.text or '')[:120],
                'last_time':    ts_val,
            ...
            c = conn.cursor()
            c.execute('UPDATE messages SET text=?, edit_date=? WHERE id=? AND chat_id=?',
                      (msg.text or '', int(msg.edit_date.timestamp()) if msg.edit_date else 0, msg.id, chat_id))
            conn.commit()
            conn.close()
            socketio.emit('message_edited', {'chat_id': str(chat_id), 'msg_id': msg.id, 'new_text': msg.text}, to=self.user_id)

        elif isinstance(update, UpdateDeleteMessages):
            if not hasattr(update, 'peer_id') or not update.peer_id:
                return
            chat_id = (getattr(update.peer_id, 'channel_id', None) or
                       getattr(update.peer_id, 'user_id', None))
            if not chat_id:
                return
            conn = sqlite3.connect(state.db_path)
            c = conn.cursor()
            for mid in update.messages:
                c.execute('DELETE FROM messages WHERE id=? AND chat_id=?', (mid, chat_id))
            conn.commit()
            conn.close()
            socketio.emit('messages_deleted', {'chat_id': str(chat_id), 'ids': update.messages}, to=self.user_id)

        elif isinstance(update, (UpdateReadHistoryInbox, UpdateReadChannelInbox)):
            if not hasattr(update, 'peer'):
                return
            chat_id = (getattr(update.peer, 'channel_id', None) or
                       getattr(update.peer, 'user_id', None) or
                       getattr(update.peer, 'chat_id', None))
            if not chat_id:
                return
            conn = sqlite3.connect(state.db_path)
            c = conn.cursor()
            c.execute('UPDATE dialogs SET unread_count=0 WHERE chat_id=?', (chat_id,))
            conn.commit()
            conn.close()
            socketio.emit('read_history', {'chat_id': str(chat_id)}, to=self.user_id)
...
def handle_disconnect(data=None):
    if 'user_id' in session:
        ...
        sid = request.sid
        leave_room(user_id)
        _remove_device_session(user_id, sid)
...
def index():
    ...
    _user_name = current_user.get('name', 'مستخدم') if isinstance(current_user, dict) else str(current_user)
...
def api_save_settings():
    ...
    current_settings.update({
        ...
        'welcome_message': data.get('welcome_message', current_settings.get('welcome_message', '')),
    })
...
@app.route("/api/get_settings_json", methods=["GET"])
def api_get_settings_json():
    """إرجاع إعدادات المستخدم الحالي بصيغة JSON لتحميلها في واجهة send_monitor"""
    if 'user_id' not in session:
    ...
    return jsonify({
        ...
        "settings": {
            ...
            "watch_words": settings.get("watch_words", []),
            "send_type": settings.get("send_type", "manual"),
            "interval_seconds": settings.get("interval_seconds", 3600),
            "schedule_duration_hours": settings.get("schedule_duration_hours", 0),
            "sanitize_mode": settings.get("sanitize_mode", "salam"),
            "max_retries": settings.get("max_retries", 5),
            "auto_reconnect": settings.get("auto_reconnect", False),
            "smart_required_messages": settings.get("smart_required_messages", 3),
        }
    })
...
def api_get_account_info():
    """
    يُعيد معلومات الحساب الحالي + قائمة بجميع الحسابات المضافة.
    الآلية الرسمية: كل حساب يحمل user_id (ليس slot رقمي) لتمرير التبديل الصحيح.
    """
    current_uid = session.get('user_id', 'user_1')
    previous_uid = session.get('previous_user_id')
    try:
        # ── معلومات الحساب الحالي ─────────────────────────────────────────
        try:
            telegram_manager.ensure_client_active(current_uid)
        except Exception as e:
            logger.debug(f"ensure_client_active in get_account_info: {e}")

        with USERS_LOCK:
            udata = USERS.get(current_uid, {})
            cached = {
        ...
        if not cached["account_name"] and cached["authenticated"]:
            try:
                cached["account_name"] = telegram_manager._fetch_account_name(current_uid)
                with USERS_LOCK:
                    cached["account_username"] = USERS.get(current_uid, {}).get('account_username')
                    cached["account_phone"]    = USERS.get(current_uid, {}).get('account_phone')
                    cached["account_avatar"]   = USERS.get(current_uid, {}).get('account_avatar')
            except Exception as e:
                logger.error(f"get_account_info refresh failed: {e}")
        if not cached.get("account_avatar"):
            avatar_file = os.path.join(SESSIONS_DIR, 'avatars', f"{current_uid}.jpg")
            if os.path.exists(avatar_file) and os.path.getsize(avatar_file) > 0:
                cached["account_avatar"] = f"/api/account_avatar/{current_uid}"
        cached["is_pro"] = is_user_restricted(current_uid)

        # ── قائمة جميع الحسابات (الآلية الرسمية) ─────────────────────────
        accounts = []
        for uid, pdata in PREDEFINED_USERS.items():
            with USERS_LOCK:
                ustate = USERS.get(uid, {})
            _auth = bool(ustate.get('authenticated', False)) or bool(load_string_session(uid))
            _avatar = ustate.get('account_avatar')
            if not _avatar:
                _av_file = os.path.join(SESSIONS_DIR, 'avatars', f"{uid}.jpg")
                if os.path.exists(_av_file) and os.path.getsize(_av_file) > 0:
                    _avatar = f"/api/account_avatar/{uid}"
            accounts.append({
                "user_id":      uid,                            # ← المعرف الصحيح للتبديل
                "name":         pdata.get('name', uid),
                "icon":         pdata.get('icon', 'fas fa-user'),
                "color":        pdata.get('color', '#6c757d'),
                "phone":        ustate.get('account_phone') or '',
                "account_name": ustate.get('account_name') or '',
                "avatar":       _avatar,
                "authenticated": _auth,
        ...
        return jsonify({
            ...
            "predefined_name":  PREDEFINED_USERS.get(current_uid, {}).get('name'),
            "accounts":         accounts,         # ← القائمة الكاملة
            **cached
        })
    except Exception as e:
...
def api_switch_user():
    try:
        ...
        # ── الآلية الرسمية: تحديد ما إذا كان الحساب الجديد بحاجة لتسجيل دخول ──
        _target_authenticated = False
        with USERS_LOCK:
            _target_authenticated = bool(USERS.get(new_user_id, {}).get('authenticated', False))
        if not _target_authenticated:
            _target_authenticated = bool(load_string_session(new_user_id))
        if not _target_authenticated:
            # الحساب الجديد غير متصل → احفظ الحالي كـ "مرجع للرجوع"
            session['previous_user_id'] = old_user_id
            logger.info(f"[MultiAccount] {old_user_id} → {new_user_id} (requires login, back-ref saved)")
        else:
            # التبديل العادي → امسح مرجع الرجوع
            session.pop('previous_user_id', None)
            logger.info(f"[MultiAccount] {old_user_id} → {new_user_id} (authenticated, no back-ref needed)")
...
# ── مسارات مختصرة للوظائف المميزة (/api/monitor/*) ──────────────────────────
@app.route("/api/monitor/save_settings", methods=["POST"])
def api_monitor_save_settings():
    """مسار مختصر يُعيد توجيه طلبات حفظ الإعدادات إلى الدالة الأصلية."""
    return api_save_settings()

@app.route("/api/monitor/send_now", methods=["POST"])
def api_monitor_send_now():
    """مسار مختصر يُعيد توجيه طلبات الإرسال الفوري إلى الدالة الأصلية."""
    return api_send_now()

@app.route("/api/monitor/start", methods=["POST"])
def api_monitor_start():
    """مسار مختصر لبدء المراقبة."""
    return api_start_monitoring()

@app.route("/api/monitor/stop", methods=["POST"])
def api_monitor_stop():
    """مسار مختصر لإيقاف المراقبة."""
    return api_stop_monitoring()

@app.route("/api/monitor/status", methods=["GET"])
def api_monitor_status():
    """إرجاع حالة المراقبة الحالية مع الإحصائيات والإعدادات المحفوظة."""
    if 'user_id' not in session:
        return jsonify({"success": False, "is_running": False, "message": "❌ غير مسجّل"})
    user_id = session['user_id']
    with USERS_LOCK:
    ...
    settings = load_settings(user_id)
    groups_val = settings.get('groups', '')
    if isinstance(groups_val, list):
        groups_val = '\n'.join(groups_val)
    watch_val = settings.get('watch_words', '')
    if isinstance(watch_val, list):
        watch_val = '\n'.join(watch_val)
    return jsonify({
        "success": True,
        "is_running": is_running,
        ...
        "settings": {
            "message":                settings.get('message', ''),
            "groups":                 groups_val,
            "watch_words":            watch_val,
            "send_type":              settings.get('send_type', 'manual'),
            "interval_seconds":       settings.get('interval_seconds', 1500),
            "schedule_duration_hours":settings.get('schedule_duration_hours', 0),
            "sanitize_mode":          settings.get('sanitize_mode', 'salam'),
            "send_to_all":            settings.get('send_to_all', False),
        }
    })
# ─────────────────────────────────────────────────────────────────────────────
...
def api_get_login_status():
    ...
    # ── محاولة استعادة الجلسة تلقائياً إن وُجدت ملفات StringSession ──
    str_session_file = os.path.join(SESSIONS_DIR, f"{user_id}_string.txt")
    has_saved_session = os.path.exists(str_session_file)

    with USERS_LOCK:
        user_in_memory = user_id in USERS
        client_manager_exists = USERS.get(user_id, {}).get('client_manager') is not None

    if has_saved_session and not client_manager_exists:
        # أطلق الاستعادة في الخلفية
        try:
            telegram_manager.ensure_client_active(user_id)
        except Exception as _e:
            logger.debug(f"ensure_client_active in get_login_status: {_e}")
    ...
    with USERS_LOCK:
        if user_id in USERS:
            ...
            if not authenticated:
                # افحص وجود ملف الجلسة (string أو sqlite)
                if has_saved_session:
                    authenticated = True
                    connected = True
                    USERS[user_id]['authenticated'] = True
                    USERS[user_id]['connected'] = True
                elif 'settings' in user_data and 'phone' in user_data['settings']:
                    session_file = os.path.join(SESSIONS_DIR, f"{user_id}_session.session")
                    if os.path.exists(session_file):
                        authenticated = True
                        connected = True
    ...
    # المستخدم ليس في USERS (إعادة تشغيل الخادم) — تحقق من وجود ملف الجلسة
    if has_saved_session:
        try:
            telegram_manager.ensure_client_active(user_id)
        except Exception:
...
def api_go_back():
    """
    الآلية الرسمية: إلغاء محاولة تسجيل الدخول والعودة إلى الحساب السابق.
    يُستدعى من زر "العودة إلى الحساب السابق" في شاشة تسجيل الدخول.
    """
    prev_id = session.pop('previous_user_id', None)
    current_id = session.get('user_id')

    if not prev_id:
        return jsonify({"success": False, "message": "لا يوجد حساب سابق للعودة إليه"})

    if prev_id not in PREDEFINED_USERS:
        return jsonify({"success": False, "message": f"الحساب السابق ({prev_id}) غير موجود"})

    # إعادة الجلسة إلى الحساب السابق
    session['user_id'] = prev_id
    session.permanent = True

    logger.info(f"[MultiAccount] go_back: {current_id} → {prev_id}")

    return jsonify({
        "success": True,
        "user_id": prev_id,
        "name": PREDEFINED_USERS[prev_id].get('name', prev_id),
        "message": f"✅ تم العودة إلى {PREDEFINED_USERS[prev_id].get('name', prev_id)}"
    })
...
def api_add_account_slot():
    ...
    try:
        ...
        if _ok:
            PREDEFINED_USERS = load_dynamic_users()
        # ── الآلية الرسمية: لا نُبدّل session هنا إطلاقاً ──
        # الفتحة تُضاف فقط كـ Entity في قاعدة البيانات.
        # التبديل يحدث لاحقاً عندما يختار المستخدم الحساب من القائمة.
        return jsonify({
            "success": True,
            "user_id": new_uid,
            ...
            "message": f"✅ تم إنشاء فتحة حساب {n} — اختره من القائمة لتسجيل الدخول",
...
def api_get_conversations():
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'غير مصرح'})
    user_id = session['user_id']
    state = SyncState(user_id)
    dialogs = state.get_dialogs_ordered(limit=100)
    # إذا كانت قاعدة البيانات فارغة، نحاول الجلب من تيليجرام مباشرة
    if not dialogs:
        with USERS_LOCK:
            client_mgr = USERS.get(user_id, {}).get('client_manager')
        if not client_mgr or not getattr(client_mgr, 'client', None):
            try:
                telegram_manager.ensure_client_active(user_id)
                # انتظار أقصى 5 ثوانٍ (بدلاً من 12) مع خروج فوري عند الجاهزية
                import time as _time
                for _ in range(5):
                    _time.sleep(1)
                    with USERS_LOCK:
                        client_mgr = USERS.get(user_id, {}).get('client_manager')
                    if client_mgr and getattr(client_mgr, 'client', None):
                        break
            except Exception as _e:
                logger.warning(f"ensure_client_active in get_conversations failed: {_e}")
            with USERS_LOCK:
                client_mgr = USERS.get(user_id, {}).get('client_manager')
        if client_mgr and getattr(client_mgr, 'client', None):
            try:
                raw_dialogs = client_mgr.run_coroutine(client_mgr.client.get_dialogs(limit=100))
                result = []
                ...
                return jsonify({'success': True, 'dialogs': result, 'conversations': result})
            except Exception as e:
                logger.error(f'api_get_conversations fallback error: {e}')
                return jsonify({'success': False, 'message': str(e)})
        return jsonify({'success': False, 'message': 'العميل غير متصل — يرجى تسجيل الدخول أولاً'})
    return jsonify({'success': True, 'dialogs': dialogs, 'conversations': dialogs})
...
def api_get_messages():
    ...
    # أولاً: اقرأ من قاعدة البيانات المحلية (أسرع وأوفر)
    if filter_type == 'all':
        try:
            state = SyncState(user_id)
            messages = state.get_messages(int(chat_id_raw), limit)
            if messages:
                return jsonify({'success': True, 'messages': messages})
        except Exception as _db_e:
            logger.debug(f'SQLite get_messages fallback triggered: {_db_e}')
    # ثانياً: الرجوع لتيليجرام إذا كانت DB فارغة أو طُلب فلتر وسائط
    with USERS_LOCK:
        client_mgr = USERS.get(user_id, {}).get('client_manager')
    if not client_mgr or not getattr(client_mgr, 'client', None):
        return jsonify({'success': False, 'message': 'العميل غير متصل'})
        ...
        async def _fetch():
            ...
            if filter_type != 'all':
                try:
                    ...
                    msg_filter = _filter_map.get(filter_type)
                except ImportError:
                    pass
            kwargs = {'limit': limit}
            if msg_filter:
                kwargs['filter'] = msg_filter
            msgs = await client_mgr.client.get_messages(entity, **kwargs)
            result = []
...
def api_get_folders():
    ...
    return jsonify({"success": True, "folders": _load_folders()})
        ...
        for f in folders:
            if f['id'] == folder_id:
                f.update({
                    ...
                    'chat_ids': data.get('chat_ids', f.get('chat_ids', [])),
                    'member_ids': data.get('member_ids', f.get('member_ids', [])),
                })
                break
        else:
...
def api_delete_folder(folder_id):
    if 'user_id' not in session:
        return jsonify({"success": False, "message": "غير مصرح"}), 401
    folders = [f for f in _load_folders() if f['id'] != folder_id]
    _save_folders(folders)
    return jsonify({"success": True, "folders": folders})

# ─── الرسائل المحفوظة (Bookmarks) ─────────────────────────────────────────────

BOOKMARKS_FILE = os.path.join('data', 'bookmarks.json')

def _load_bookmarks(user_id):
    try:
        if os.path.exists(BOOKMARKS_FILE):
            with open(BOOKMARKS_FILE, 'r', encoding='utf-8') as f:
                all_bm = json.load(f)
                return all_bm.get(user_id, [])
    except Exception:
        pass
    return []

def _save_bookmarks(user_id, bookmarks):
    try:
        os.makedirs('data', exist_ok=True)
        all_bm = {}
        if os.path.exists(BOOKMARKS_FILE):
            with open(BOOKMARKS_FILE, 'r', encoding='utf-8') as f:
                all_bm = json.load(f)
        all_bm[user_id] = bookmarks
        with open(BOOKMARKS_FILE, 'w', encoding='utf-8') as f:
            json.dump(all_bm, f, ensure_ascii=False, indent=2)
    except Exception:
        pass
...
def api_get_bookmarks():
    if 'user_id' not in session:
        return jsonify({"success": False, "bookmarks": []}), 401
    return jsonify({"success": True, "bookmarks": _load_bookmarks(session['user_id'])})
...
def api_add_bookmark():
    ...
    # لا تكرار
    if any(b.get('message_id') == msg_id for b in bookmarks):
        return jsonify({"success": True, "message": "محفوظ مسبقاً"})
    bookmarks.insert(0, {
        'message_id': msg_id,
        'text': data.get('text', ''),
    ...
    _save_bookmarks(user_id, bookmarks[:200])  # حد أقصى 200 رسالة
    return jsonify({"success": True, "message": "تم الحفظ"})

# ─── إدارة الجلسات — /api/auth ────────────────────────────────────────────────

@app.route("/api/auth/revoke_all_sessions", methods=["POST"])
def api_revoke_all_sessions():
    if 'user_id' not in session:
        return jsonify({"success": False, "message": "غير مصرح"}), 401
    # تفويض للدالة الموجودة
    from flask import current_app
    try:
        user_id = session['user_id']
        with USERS_LOCK:
            cm = USERS.get(user_id, {}).get('client_manager')
        if cm and hasattr(cm, 'client') and cm.client:
            async def _revoke():
                from telethon.tl.functions.auth import ResetAuthorizationsRequest
                await cm.client(ResetAuthorizationsRequest())
            cm.run_coroutine(_revoke())
            return jsonify({"success": True, "message": "تم إنهاء جميع الجلسات الأخرى"})
    except Exception as e:
        logger.error(f"revoke_all_sessions error: {e}")
    return jsonify({"success": False, "message": "تعذّر إنهاء الجلسات — تأكد من الاتصال"})

@app.route("/api/auth/revoke_session", methods=["POST"])
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _revoke():
                ...
                await cm.client(ResetAuthorizationRequest(hash=int(data.get('session_id', 0))))
            cm.run_coroutine(_revoke())
            return jsonify({"success": True, "message": "تم إنهاء الجلسة"})
    except Exception as e:
        logger.error(f"revoke_session error: {e}")
    return jsonify({"success": False, "message": "تعذّر إنهاء الجلسة"})

# ─── إعدادات عامة — /api/settings ────────────────────────────────────────────

@app.route("/api/settings", methods=["GET"])
def api_settings_get():
    if 'user_id' not in session:
        return jsonify({"success": False}), 401
    s = load_settings(session['user_id'])
...
# ═══════════════════════════════════════════════════════════════════════════
# مسارات API المفقودة — المراحل 7-12
# ═══════════════════════════════════════════════════════════════════════════
...
def api_msg_reaction():
    ...
    try:
        store = json.loads(open(path).read()) if os.path.exists(path) else {}
    except Exception:
        store = {}
    key = str(msg_id)
    if key not in store:
        store[key] = {}
    if emoji not in store[key]:
        store[key][emoji] = []
    if user_id in store[key][emoji]:
        store[key][emoji].remove(user_id)
        added = False
    else:
        store[key][emoji].append(user_id)
        added = True
    # نظّف الفارغة
    store[key] = {e: ul for e, ul in store[key].items() if ul}
    try:
        with open(path, 'w', encoding='utf-8') as f:
            json.dump(store, f, ensure_ascii=False)
    except Exception:
        pass
    socketio.emit('message_reaction_update', {'message_id': msg_id, 'reactions': store.get(key, {})})
    return jsonify({"success": True, "reactions": store.get(key, {}), "added": added})
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _pin():
                ...
                await cm.client(UpdatePinnedMessageRequest(peer=int(chat_id), id=int(msg_id), silent=True))
            cm.run_coroutine(_pin())
            return jsonify({"success": True})
    except Exception as e:
        logger.error(f"pin_message error: {e}")
    return jsonify({"success": False, "message": str(e) if 'e' in dir() else "فشل التثبيت"})
...
def api_forward_messages():
    ...
    user_id      = session['user_id']
    if not from_chat_id or not to_chat_id or not message_ids:
        return jsonify({"success": False, "message": "بيانات ناقصة"})
    try:
        with USERS_LOCK:
            cm = USERS.get(user_id, {}).get('client_manager')
        if cm and hasattr(cm, 'client') and cm.client:
            async def _fwd():
                ...
                await cm.client(ForwardMessagesRequest(
                    from_peer=int(from_chat_id),
                    id=[int(i) for i in message_ids],
                    to_peer=int(to_chat_id),
                    silent=True
                ))
            cm.run_coroutine(_fwd())
            return jsonify({"success": True})
    except Exception as e:
        logger.error(f"forward_messages error: {e}")
    return jsonify({"success": False, "message": "فشل التحويل"})

# ─── ترجمة (انظر /api/translate أدناه — Groq-based) ────────────────────────
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _search():
                ...
                if chat_id:
                    msgs = await cm.client.get_messages(int(chat_id), search=q, limit=30)
                else:
                    msgs = await cm.client.get_messages(None, search=q, limit=20)
                return [{
                    'id': str(m.id), 'chat_id': str(m.peer_id.channel_id if hasattr(m.peer_id,'channel_id') else getattr(m.peer_id,'user_id',0)),
                    'text': m.text or '', 'timestamp': m.date.timestamp() if m.date else 0,
                    'sender_name': getattr(m.sender, 'first_name', '') if m.sender else 'مستخدم'
                } for m in msgs if m.text]
            results = cm.run_coroutine(_search()) or []
    except Exception as e:
...
def api_send_media():
    ...
    import tempfile, uuid, threading

    # احفظ الملفات مؤقتاً قبل تشغيل الخيط (request context ينتهي بعد الإرجاع)
    saved_files = []
    for file in files:
    ...
    def _upload_worker(uid, cid, paths, tid):
        """يرفع الملفات في الخلفية ويُبلّغ عبر Socket.IO عند الانتهاء."""
        sent_ids = []
        try:
            with USERS_LOCK:
                cm = USERS.get(uid, {}).get('client_manager')
            if cm and hasattr(cm, 'client') and cm.client:
                for path in paths:
                    try:
                        async def _send(p=path):
                            msg = await cm.client.send_file(int(cid), p)
                            return msg.id
                        mid = cm.run_coroutine(_send())
                        if mid:
                            sent_ids.append(mid)
                    except Exception as ex:
                        logger.error(f"send_media upload error for {path}: {ex}")
                    finally:
                        try:
                            os.unlink(path)
                        except Exception:
                            pass
            # أبلغ الواجهة بنتيجة الرفع عبر Socket.IO
            socketio.emit('media_sent', {
                'task_id': tid,
            ...
            logger.error(f"send_media worker error: {e}")
            socketio.emit('media_sent', {
                'task_id': tid,
                'success': False,
                'message': str(e),
    ...
    t = _OSThread(target=_upload_worker, args=(user_id, chat_id, saved_files, task_id), daemon=True)
    t.start()

    # يُرجع فوراً دون انتظار الرفع (يمنع Timeout على الملفات الكبيرة)
    return jsonify({"success": True, "uploading": True, "task_id": task_id,
                    "message": "⏳ جاري رفع الملف في الخلفية..."})

# ─── تنزيل الوسائط ──────────────────────────────────────────────────────────
@app.route("/api/media/<int:chat_id>/<int:msg_id>", methods=["GET"])
def api_get_media(chat_id, msg_id):
    if 'user_id' not in session:
        return jsonify({"success": False, "message": "غير مصرح"}), 401
    user_id = session['user_id']
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _dl():
                msg = await cm.client.get_messages(chat_id, ids=msg_id)
                if not msg or not msg.media:
                    return None, None
                with tempfile.NamedTemporaryFile(delete=False) as tmp:
                    path = await cm.client.download_media(msg, file=tmp.name)
                    with open(path, 'rb') as f:
                        b64 = base64.b64encode(f.read()).decode()
                    mime = getattr(msg.media, 'document', None)
                    mime_type = getattr(getattr(mime, 'mime_type', None), '__str__', lambda: 'application/octet-stream')()
                    try:
                        os.unlink(path)
                    except Exception:
                        pass
                    return b64, mime_type
            data_b64, mime = cm.run_coroutine(_dl()) or (None, None)
            if data_b64:
                return jsonify({"success": True, "data": data_b64, "mime": mime or "application/octet-stream"})
    except Exception as e:
        logger.error(f"get_media error: {e}")
    return jsonify({"success": False, "message": "فشل تحميل الوسيط"})

# ─── حالة المحادثات (أرشفة / كتم) ──────────────────────────────────────────
@app.route("/api/chats/states", methods=["GET"])
def api_chats_states():
    if 'user_id' not in session:
        return jsonify({"success": False}), 401
    user_id = session['user_id']
    path    = os.path.join('data', f'chat_states_{user_id}.json')
    try:
        s = json.loads(open(path).read()) if os.path.exists(path) else {}
    except Exception:
        s = {}
    return jsonify({"success": True, "archived": s.get('archived', []), "muted": s.get('muted', [])})
...
def api_archive_chat(chat_id):
    ...
    if action == 'archive' and chat_id not in archived:
        archived.append(chat_id)
    elif action == 'unarchive' and chat_id in archived:
        archived.remove(chat_id)
    s['archived'] = archived
    try:
        with open(path, 'w', encoding='utf-8') as f:
            json.dump(s, f)
...
def api_mute_chat(chat_id):
    ...
    try:
        ...
        s = {}
    muted = s.get('muted', [])
    if action == 'mute' and chat_id not in muted:
        muted.append(chat_id)
    elif action == 'unmute' and chat_id in muted:
...
@app.route("/api/chats/<int:chat_id>/notify_settings", methods=["GET", "POST"])
def api_chat_notify_settings(chat_id):
    if 'user_id' not in session:
        return jsonify({"success": False}), 401
    user_id = session['user_id']
    path    = os.path.join('data', f'notify_settings_{user_id}.json')
    try:
        store = json.loads(open(path).read()) if os.path.exists(path) else {}
    except Exception:
        store = {}
    ...
    if request.method == 'GET':
        return jsonify({"success": True, "settings": store.get(key, {"sound": True, "show_preview": True})})
    data   = request.get_json(force=True, silent=True) or {}
    if key not in store:
        store[key] = {"sound": True, "show_preview": True}
...
@app.route("/api/chats/<int:chat_id>/export", methods=["POST"])
def api_export_chat(chat_id):
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _export():
                msgs = await cm.client.get_messages(chat_id, limit=200)
                return [{"id": m.id, "text": m.text or '', "date": str(m.date),
                         "sender": getattr(m.sender, 'first_name', 'مستخدم') if m.sender else 'مستخدم'} for m in msgs]
            messages = cm.run_coroutine(_export()) or []
    except Exception as e:
    ...
    if fmt == 'json':
        ...
        rows = ''.join(f"<tr><td>{m['date']}</td><td>{m['sender']}</td><td>{m['text']}</td></tr>" for m in messages)
        html = f"<html><body><table><tr><th>التاريخ</th><th>المرسل</th><th>الرسالة</th></tr>{rows}</table></body></html>"
...
def api_load_more_messages(chat_id):
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _more():
                msgs = await cm.client.get_messages(chat_id, limit=limit, offset_id=offset_id, reverse=False)
                result = []
                for m in msgs:
                    if not m:
                        continue
                    result.append({
                        'id': m.id, 'text': m.text or '', 'date': m.date.timestamp() if m.date else 0,
                        'out': m.out, 'sender_id': getattr(m.sender_id, 'user_id', m.sender_id) if m.sender_id else None,
                        'sender_name': getattr(m.sender, 'first_name', 'مستخدم') if m.sender else 'مستخدم',
                    })
                return result
            msgs = cm.run_coroutine(_more()) or []
            return jsonify({"success": True, "messages": msgs})
    except Exception as e:
        logger.error(f"load_more_messages error: {e}")
    return jsonify({"success": False, "messages": []})

# ─── حظر المستخدمين ─────────────────────────────────────────────────────────
@app.route("/api/users/<int:user_id_target>/block", methods=["POST", "DELETE"])
def api_block_user(user_id_target):
    if 'user_id' not in session:
        return jsonify({"success": False, "message": "غير مصرح"}), 401
    user_id = session['user_id']
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _block():
                ...
                if block:
                    await cm.client(BlockRequest(id=user_id_target))
                else:
                    await cm.client(UnblockRequest(id=user_id_target))
            cm.run_coroutine(_block())
            return jsonify({"success": True})
    except Exception as e:
        logger.error(f"block_user error: {e}")
    return jsonify({"success": False, "message": str(e) if 'e' in dir() else "فشلت العملية"})

# ─── معلومات الملف الشخصي ────────────────────────────────────────────────────
@app.route("/api/profile/<int:chat_id>", methods=["GET"])
def api_get_profile(chat_id):
    if 'user_id' not in session:
        return jsonify({"success": False}), 401
    user_id = session['user_id']
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _profile():
                entity = await cm.client.get_entity(chat_id)
                return {
                    'id': entity.id,
                    'username': getattr(entity, 'username', ''),
                    'phone': getattr(entity, 'phone', ''),
                    'bio': getattr(getattr(entity, 'about', None), '__str__', lambda: '')(),
                }
            p = cm.run_coroutine(_profile()) or {}
...
def api_schedule_message():
    ...
    user_id      = session['user_id']
    if not chat_id or not text or not scheduled_at:
        return jsonify({"success": False, "message": "بيانات ناقصة"})
    path = os.path.join('data', f'scheduled_{user_id}.json')
    try:
        store = json.loads(open(path).read()) if os.path.exists(path) else []
    except Exception:
        store = []
    import uuid as _uuid
    store.append({"id": str(_uuid.uuid4())[:8], "chat_id": chat_id, "text": text, "scheduled_at": scheduled_at})
    try:
        with open(path, 'w', encoding='utf-8') as f:
            json.dump(store, f, ensure_ascii=False)
    except Exception:
...
@app.route("/api/get_scheduled_messages", methods=["GET"])
def api_get_scheduled_messages():
    if 'user_id' not in session:
        return jsonify({"success": False}), 401
    user_id = session['user_id']
    chat_id = request.args.get('chat_id')
    ...
    try:
        ...
        store = []
    if chat_id:
        store = [m for m in store if str(m.get('chat_id')) == str(chat_id)]
    return jsonify({"success": True, "messages": store})

@app.route("/api/delete_scheduled_message", methods=["POST"])
def api_delete_scheduled_message():
    ...
    try:
        store = json.loads(open(path).read()) if os.path.exists(path) else []
        store = [m for m in store if m.get('id') != msg_id]
        with open(path, 'w', encoding='utf-8') as f:
            json.dump(store, f, ensure_ascii=False)
    except Exception:
        pass
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _poll():
                ...
                poll = InputMediaPoll(poll=Poll(
                    id=0, question=question, answers=[PollAnswer(text=o, option=str(i).encode()) for i, o in enumerate(options)],
                    public_voters=not is_anonymous, multiple_choice=False, quiz=False
                ))
                await cm.client.send_file(int(chat_id), poll)
            cm.run_coroutine(_poll())
            return jsonify({"success": True})
    except Exception as e:
        logger.error(f"create_poll error: {e}")
        return jsonify({"success": False, "message": str(e)})
    return jsonify({"success": False, "message": "فشل إنشاء الاستطلاع"})
...
def api_privacy_settings():
    if 'user_id' not in session:
        return jsonify({"success": False}), 401
    user_id = session['user_id']
    path    = os.path.join('data', f'privacy_{user_id}.json')
    if request.method == 'GET':
        try:
            s = json.loads(open(path).read()) if os.path.exists(path) else {}
        except Exception:
...
def api_storage_stats():
    ...
    def _dir_size(d):
        ...
        try:
            ...
            pass
        return total
    msgs_size  = _dir_size(data_dir)
    sess_size  = _dir_size(sessions_dir) if os.path.exists(sessions_dir) else 0
    cache_size = 0
    try:
        ...
        pass
    total = msgs_size + sess_size + cache_size
...
def api_calls_log():
    ...
    data    = request.get_json(force=True, silent=True) or {}
    user_id = session['user_id']
    path    = os.path.join('data', f'calls_{user_id}.json')
    try:
        store = json.loads(open(path).read()) if os.path.exists(path) else []
    except Exception:
        store = []
    import time as _time
    store.insert(0, {
        "peer_id": data.get('peer_id'), "peer_name": data.get('peer_name', ''),
        "direction": data.get('direction', 'outgoing'), "status": data.get('status', 'ended'),
        "duration": data.get('duration', 0), "started_at": _time.time()
    })
    store = store[:100]  # احتفظ بآخر 100 مكالمة
    try:
        with open(path, 'w', encoding='utf-8') as f:
            json.dump(store, f, ensure_ascii=False)
    except Exception:
...
def api_calls_history():
    ...
    try:
        ...
        store = []
    return jsonify({"success": True, "calls": store[:limit]})
...
def api_2fa_status():
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _check():
                pwd = await cm.client(cm.client._get_account_password_async() if hasattr(cm.client, '_get_account_password_async') else __import__('telethon').tl.functions.account.GetPasswordRequest())
                return getattr(pwd, 'has_password', False)
            has_2fa = cm.run_coroutine(_check()) or False
    except Exception:
        pass
    return jsonify({"success": True, "has_2fa": has_2fa})
...
def api_2fa_enable():
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _enable():
                await cm.client.edit_2fa(new_password=pwd, hint=hint)
            cm.run_coroutine(_enable())
            return jsonify({"success": True})
    except Exception as e:
        logger.error(f"2fa_enable error: {e}")
        return jsonify({"success": False, "message": str(e)})
    return jsonify({"success": False, "message": "فشل التفعيل"})
...
def api_2fa_disable():
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _disable():
                await cm.client.edit_2fa(current_password=cur_pwd, new_password=None)
            cm.run_coroutine(_disable())
            return jsonify({"success": True})
    except Exception as e:
        logger.error(f"2fa_disable error: {e}")
        return jsonify({"success": False, "message": str(e)})
    return jsonify({"success": False, "message": "فشل التعطيل"})
...
def api_sync_status():
    if 'user_id' not in session:
        return jsonify({"success": False}), 401
    from datetime import datetime as _dt
    return jsonify({"success": True, "last_sync": _dt.now().strftime('%H:%M')})
...
def api_sync_import():
    ...
    if isinstance(payload, dict):
        s.update(payload)
        save_settings(user_id, s)
        return jsonify({"success": True, "imported": len(payload)})
    return jsonify({"success": False, "message": "بيانات غير صالحة"})
...
def api_get_bots():
    if 'user_id' not in session:
        return jsonify({"success": False}), 401
    path = os.path.join('data', 'bots.json')
    try:
        bots = json.loads(open(path).read()) if os.path.exists(path) else []
    except Exception:
        bots = []
...
def api_add_bot():
    ...
    if any(b['name'] == name for b in bots):
        return jsonify({"success": False, "message": "البوت موجود بالفعل"})
    bots.append({"name": name, "phone": phone, "is_active": True})
    try:
        with open(path, 'w', encoding='utf-8') as f:
...
def api_bot_message(bot_name):
    ...
    data = request.get_json(force=True, silent=True) or {}
    text = data.get('text', '')
    return jsonify({"success": True, "reply": f"البوت [{bot_name}] استقبل: {text}"})
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _contacts():
                ...
                result = await cm.client(GetContactsRequest(hash=0))
                return [{"id": u.id, "name": (u.first_name or '') + ' ' + (u.last_name or ''), "phone": u.phone or ''} for u in result.users]
            contacts = cm.run_coroutine(_contacts()) or []
    except Exception as e:
        logger.error(f"get_contacts error: {e}")
    if q:
        contacts = [c for c in contacts if q in c['name'].lower() or q in c['phone']]
    return jsonify({"success": True, "contacts": contacts[:50]})
...
def api_create_group():
    ...
    t       = data.get('type', 'group')
    users   = [u.strip() for u in data.get('users', '').split(',') if u.strip()]
    user_id = session['user_id']
    if not title:
        return jsonify({"success": False, "message": "الاسم مطلوب"})
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _create():
                if t == 'channel':
                    ...
                    result = await cm.client(CreateChannelRequest(title=title, about='', broadcast=True))
                elif t == 'megagroup':
                    from telethon.tl.functions.channels import CreateChannelRequest
                    result = await cm.client(CreateChannelRequest(title=title, about='', megagroup=True))
                else:
                    from telethon.tl.functions.messages import CreateChatRequest
                    result = await cm.client(CreateChatRequest(title=title, users=users or []))
                return True
            cm.run_coroutine(_create())
            return jsonify({"success": True, "message": f'تم إنشاء "{title}"'})
    except Exception as e:
        logger.error(f"create_group error: {e}")
        return jsonify({"success": False, "message": str(e)})
    return jsonify({"success": False, "message": "فشل الإنشاء"})

# ─── مزيد من مسارات الجلسات ─────────────────────────────────────────────────
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _sessions():
                ...
                result = await cm.client(GetAuthorizationsRequest())
                return [{
                    'hash': getattr(a, 'hash', 0),
                    'device': getattr(a, 'device_model', 'Unknown'),
                    'platform': getattr(a, 'platform', ''),
                    'app': getattr(a, 'app_name', 'Telegram'),
            ...
            sessions = cm.run_coroutine(_sessions()) or []
    except Exception as e:
        logger.error(f"auth_sessions error: {e}")
        sessions = [{'hash': 0, 'device': 'هذا الجهاز', 'current': True, 'app': 'Telegram Web'}]
    return jsonify({"success": True, "sessions": sessions})


# ══════════════════════════════════════════════════════════════
#  لوحة التحكم — يعرض التابات المختلفة (متسلسل/تعلم/ردود/روابط)
# ══════════════════════════════════════════════════════════════
...
def link_finder_underscore_page():
    if 'user_id' not in session:
        return redirect('/')
    return render_template('link_finder.html')
...
# ════════════════════════════════════════════════════════════════
#  🌐  الترجمة الحقيقية (باستخدام Groq)
# ════════════════════════════════════════════════════════════════
...
def api_translate():
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": f"أنت مترجم محترف. ترجم النص التالي إلى اللغة العربية ({target_lang}) بدقة. أخرج النص المترجم فقط دون أي شرح إضافي."},
        ...
        return jsonify({'success': True, 'translated': translated, 'source': 'groq'})
    except Exception as e:
        logger.warning(f"Groq translation failed: {e}, falling back to Google")
        try:
            from googletrans import Translator
            translator = Translator()
            result = translator.translate(text, dest=target_lang)
            return jsonify({'success': True, 'translated': result.text, 'source': 'google'})
        except Exception as e2:
            return jsonify({'success': False, 'message': f'فشلت الترجمة: {str(e2)}'}), 500
...
def api_storage_settings():
    ...
    if request.method == 'GET':
        ...
        return jsonify({'success': True, 'settings': storage})
    else:
        data = request.get_json(force=True, silent=True) or {}
        ...
        save_settings(user_id, settings)
        return jsonify({'success': True, 'message': 'تم حفظ إعدادات التخزين'})


@app.route('/api/storage/clear_cache', methods=['POST'])
def api_clear_cache():
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'غير مصرح'}), 401
    user_id = session['user_id']
    try:
        import shutil
        cache_dir = os.path.join(SESSIONS_DIR, 'cache', user_id)
...
# ════════════════════════════════════════════════════════════════════
#  🚀 تكامل TDLib — مسارات API الجديدة
#  يعمل جنباً إلى جنب مع Telethon دون أي تعارض
# ════════════════════════════════════════════════════════════════════
try:
    from config_tdlib import tdlib_manager as _tdlib_mgr
    from auth_tdlib import TDLibAuth as _TDLibAuth
    from chat_manager_tdlib import ChatManagerTDLib as _ChatMgrTDLib
    from sync_handler_tdlib import tdlib_sync as _tdlib_sync
    from legacy_integration import LegacyIntegration as _LegacyInteg
    _TDLIB_ROUTES_ENABLED = True
    # بدء نظام المزامنة مع Socket.IO
    _tdlib_sync.start(socketio)
except Exception as _tdlib_import_err:
    _TDLIB_ROUTES_ENABLED = False
    logger.warning(f"TDLib integration disabled: {_tdlib_import_err}")

# ── مصادقة TDLib ────────────────────────────────────────────────────
@app.route('/api/tdlib/login', methods=['POST'])
def tdlib_login():
    if not _TDLIB_ROUTES_ENABLED:
        return jsonify({'success': False, 'message': 'TDLib غير متاح في هذه البيئة'})
    user_id = session.get('user_id', 'user_1')
    data = request.get_json(force=True, silent=True) or {}
    phone = data.get('phone', '').strip()
    if not phone:
        return jsonify({'success': False, 'message': 'رقم الهاتف مطلوب'})
    auth = _TDLibAuth(user_id)
    auth.set_socketio(socketio)
    result = auth.start_auth(phone)
    session['tdlib_auth_state'] = auth.auth_state
    return jsonify(result)

@app.route('/api/tdlib/verify_code', methods=['POST'])
def tdlib_verify_code():
    if not _TDLIB_ROUTES_ENABLED:
        return jsonify({'success': False, 'message': 'TDLib غير متاح'}), 400
    user_id = session.get('user_id', 'user_1')
    code = (request.get_json(force=True, silent=True) or {}).get('code', '').strip()
    if not code:
        return jsonify({'success': False, 'message': 'الكود مطلوب'})
    auth = _TDLibAuth(user_id)
    auth.auth_state = 'code'
    return jsonify(auth.verify_code(code))

@app.route('/api/tdlib/verify_password', methods=['POST'])
def tdlib_verify_password():
    if not _TDLIB_ROUTES_ENABLED:
        return jsonify({'success': False, 'message': 'TDLib غير متاح'}), 400
    user_id = session.get('user_id', 'user_1')
    ...
    auth = _TDLibAuth(user_id)
    auth.auth_state = 'password'
    return jsonify(auth.verify_password(password))

@app.route('/api/tdlib/status', methods=['GET'])
def tdlib_status():
    """حالة TDLib — متاح أم لا"""
    available = _TDLIB_ROUTES_ENABLED and _tdlib_mgr.available if _TDLIB_ROUTES_ENABLED else False
    user_id = session.get('user_id', 'user_1')
    return jsonify({
        'available': available,
        'authenticated': _tdlib_mgr.is_authenticated(user_id) if available else False,
        'user_id': user_id,
    })

# ── محادثات TDLib ───────────────────────────────────────────────────
@app.route('/api/tdlib/chats', methods=['GET'])
def tdlib_chats():
    if not _TDLIB_ROUTES_ENABLED:
        return jsonify({'success': False, 'message': 'TDLib غير متاح'})
    user_id = session.get('user_id', 'user_1')
    limit = int(request.args.get('limit', 100))
    cm = _ChatMgrTDLib(user_id)
    cm.load_chats(limit=limit)
    return jsonify({'success': True, 'message': 'جاري تحميل المحادثات عبر TDLib'})

@app.route('/api/tdlib/chats/<int:chat_id>/messages', methods=['GET'])
def tdlib_chat_messages(chat_id):
    if not _TDLIB_ROUTES_ENABLED:
        return jsonify({'success': False, 'message': 'TDLib غير متاح'})
    user_id = session.get('user_id', 'user_1')
    limit = int(request.args.get('limit', 50))
    cm = _ChatMgrTDLib(user_id)
    cm.load_messages(chat_id, limit=limit)
    return jsonify({'success': True, 'message': 'جاري تحميل الرسائل عبر TDLib'})

@app.route('/api/tdlib/send', methods=['POST'])
def tdlib_send_message():
    if not _TDLIB_ROUTES_ENABLED:
        return jsonify({'success': False, 'message': 'TDLib غير متاح'})
    user_id = session.get('user_id', 'user_1')
    data = request.get_json(force=True, silent=True) or {}
    ...
    cm = _ChatMgrTDLib(user_id)
    ok = cm.send_message(int(chat_id), text)
    return jsonify({'success': ok})

# ── وظائف الإرسال الجماعي عبر TDLib (send_monitor) ─────────────────
@app.route('/api/tdlib/send_bulk', methods=['POST'])
def tdlib_send_bulk():
    if not _TDLIB_ROUTES_ENABLED:
        return jsonify({'success': False, 'message': 'TDLib غير متاح'})
    user_id = session.get('user_id', 'user_1')
    ...
    if not groups or not message:
        return jsonify({'success': False, 'message': 'المجموعات والرسالة مطلوبة'})
    leg = _LegacyInteg(user_id)
    results = leg.send_to_groups(groups, message, delay=delay)
    return jsonify({'success': True, 'results': results})

# ── الانضمام للمجموعات عبر TDLib (auto_join) ────────────────────────
@app.route('/api/tdlib/join_groups', methods=['POST'])
def tdlib_join_groups():
    if not _TDLIB_ROUTES_ENABLED:
        return jsonify({'success': False, 'message': 'TDLib غير متاح'})
    user_id = session.get('user_id', 'user_1')
    data = request.get_json(force=True, silent=True) or {}
    ...
    leg = _LegacyInteg(user_id)
    results = leg.join_groups_batch(links, delay=delay)
    return jsonify({'success': True, 'results': results})
...
# ════════════════════════════════════════════════════════════════════
#  الإضافات الـ 19 الجديدة — تكامل مع نظام التليجرام الموجود
# ════════════════════════════════════════════════════════════════════

# ── 1. المحادثات السرية (E2EE) ──────────────────────────────────
@app.route('/api/start_secret_chat', methods=['POST'])
def api_start_secret_chat():
    user_id = session.get('user_id')
    if not user_id:
        return jsonify({'success': False, 'message': 'غير مصرح'})
    data = request.get_json(force=True, silent=True) or {}
    ...
    if not cm or not cm.client:
        return jsonify({'success': False, 'message': 'العميل غير متصل'})
    try:
        entity = cm.run_coroutine(cm.client.get_entity(contact))
        name = getattr(entity, 'first_name', '') or getattr(entity, 'title', contact)
        return jsonify({
            'success': True,
            'message': f'✅ يمكنك الآن التحدث مع {name} — المحادثات السرية E2EE تتطلب Telegram Desktop'
        })
    except Exception as e:
        logger.error(f"Secret chat error: {e}")
        return jsonify({'success': False, 'message': str(e)})


# ── 3. الإبلاغ عن المحتوى ────────────────────────────────────────
...
def api_report_message():
    ...
    try:
        ...
        async def _report():
            entity = await cm.client.get_entity(int(chat_id))
            await cm.client(ReportRequest(
                peer=entity,
                id=[int(msg_id)],
                reason=reason_map.get(reason, InputReportReasonSpam()),
                message=comment or 'تم الإبلاغ عن هذه الرسالة'
            ))
        cm.run_coroutine(_report())
...
def api_send_voice():
    ...
    try:
        ...
        with _tmpfile.NamedTemporaryFile(delete=False, suffix='.webm') as tmp:
            file.save(tmp.name)
            tmp_path = tmp.name
        async def _send_voice():
            entity = await cm.client.get_entity(int(chat_id))
            sent = await cm.client.send_file(entity, tmp_path, voice_note=True)
            return sent.id
        msg_id = cm.run_coroutine(_send_voice())
        os.unlink(tmp_path)
...
def api_get_audio(msg_id):
    ...
    try:
        async def _dl():
            if chat_id:
                entity = await cm.client.get_entity(int(chat_id))
                msgs = await cm.client.get_messages(entity, ids=msg_id)
            else:
                msgs = await cm.client.get_messages(None, ids=msg_id)
            if not msgs or not msgs.media:
                return None
            return await msgs.download_media(file=bytes)
        data = cm.run_coroutine(_dl())
        if not data:
            return abort(404)
...
# ── 6. صلاحيات المشرفين ─────────────────────────────────────────
@app.route('/api/admin_rights', methods=['POST'])
def api_admin_rights():
    user_id = session.get('user_id')
    if not user_id:
        return jsonify({'success': False, 'message': 'غير مصرح'})
    data = request.get_json(force=True, silent=True) or {}
    ...
    rank = data.get('rank', '')
    if not group_id or not target_user:
        return jsonify({'success': False, 'message': 'بيانات ناقصة'})
    with USERS_LOCK:
        if user_id not in USERS:
            return jsonify({'success': False, 'message': 'المستخدم غير موجود'})
    ...
    try:
        ...
        async def _set_admin():
            group = await cm.client.get_entity(int(group_id))
            user = await cm.client.get_entity(target_user)
            admin_rights = ChatAdminRights(
                change_info=rights.get('change_info', False),
                post_messages=rights.get('post_messages', False),
            ...
            await cm.client(EditAdminRequest(channel=group, user_id=user, admin_rights=admin_rights, rank=rank))
        cm.run_coroutine(_set_admin())
        return jsonify({'success': True, 'message': '✅ تم تحديث صلاحيات المشرف'})
    except Exception as e:
        logger.error(f"Admin rights error: {e}")
        return jsonify({'success': False, 'message': str(e)})


# ── 7. المواضيع الفرعية ─────────────────────────────────────────
@app.route('/api/topics/<chat_id>', methods=['GET'])
def api_get_topics(chat_id):
    user_id = session.get('user_id')
    if not user_id:
        return jsonify({'success': False, 'message': 'غير مصرح'})
    with USERS_LOCK:
    ...
    try:
        ...
        async def _topics():
            entity = await cm.client.get_entity(int(chat_id))
            result = await cm.client(GetForumTopicsRequest(channel=entity, limit=50))
            topics = []
            for t in (result.topics if hasattr(result, 'topics') else []):
                topics.append({
                    'id': t.id,
                    'title': getattr(t, 'title', ''),
                    'messages': getattr(t, 'top_message', 0),
                })
            return topics
        topics = cm.run_coroutine(_topics())
        return jsonify({'success': True, 'topics': topics})
    except Exception as e:
        logger.error(f"Topics error: {e}")
        return jsonify({'success': False, 'topics': [], 'message': str(e)})
...
def api_create_topic():
    ...
    try:
        ...
        async def _create():
            entity = await cm.client.get_entity(int(chat_id))
            result = await cm.client(CreateForumTopicRequest(channel=entity, title=title))
            return result
        cm.run_coroutine(_create())
        return jsonify({'success': True, 'message': '✅ تم إنشاء الموضوع'})
    except Exception as e:
...
# ── 8. إحصائيات القنوات ─────────────────────────────────────────
@app.route('/api/channel_stats/<chat_id>', methods=['GET'])
def api_channel_stats(chat_id):
    user_id = session.get('user_id')
    if not user_id:
        return jsonify({'success': False, 'message': 'غير مصرح'})
    with USERS_LOCK:
    ...
    try:
        async def _stats():
            entity = await cm.client.get_entity(int(chat_id))
            full = await cm.client.get_entity(entity)
            subs = getattr(getattr(full, 'full_chat', full), 'participants_count', None)
            return {
...
def api_send_sticker():
    ...
    if not chat_id or not sticker_text:
        return jsonify({'success': False, 'message': 'بيانات ناقصة'})
    with USERS_LOCK:
        if user_id not in USERS:
            return jsonify({'success': False, 'message': 'المستخدم غير موجود'})
    ...
    try:
        async def _send():
            entity = await cm.client.get_entity(int(chat_id))
            sent = await cm.client.send_message(entity, sticker_text)
            return sent.id
        msg_id = cm.run_coroutine(_send())
        return jsonify({'success': True, 'message_id': msg_id})
    except Exception as e:
...
def api_preview_link():
    ...
    try:
        ...
        title = _meta('og:title') or (soup.title.string if soup.title else url)
        description = _meta('og:description') or _meta('description')
        image = _meta('og:image')
        site_name = _meta('og:site_name')
        return jsonify({'success': True, 'title': str(title)[:150], 'description': str(description)[:300],
                        'image': image, 'site_name': site_name, 'url': url})
    except Exception as e:
        logger.error(f"Preview link error: {e}")
        return jsonify({'success': False, 'message': str(e)})
...
#  ██║  ██║╚██████╔╝╚██████╔╝   ██║   ███████╗███████║
#  ╚═╝  ╚═╝ ╚═════╝  ╚═════╝    ╚═╝   ╚══════╝╚══════╝
#  الحزمة 1: رفع الملفات المتدفق (Streaming Upload)
# ============================================================
UPLOAD_CHUNK_SIZE = 1024 * 1024  # 1 ميجابايت
...
def api_upload_chunk():
    ...
    if sess['completed']:
        return jsonify({'success': False, 'message': 'اكتمل الرفع'})

    try:
        chunk_data = chunk.read()
        with open(sess['file_path'], 'r+b') as f:
            f.seek(offset)
            f.write(chunk_data)
        uploaded_size = os.path.getsize(sess['file_path'])
        progress = int((uploaded_size / sess['size']) * 100) if sess['size'] > 0 else 0
...
def api_upload_complete():
    """إكمال الرفع وإرسال الملف إلى تيليجرام"""
    data = request.get_json(force=True, silent=True) or {}
    ...
    user_id = sess.get('user_id') or session.get('user_id')
    ...
    try:
        async def _send_file():
            entity = await cm.client.get_entity(int(chat_id))
            sent = await cm.client.send_file(entity, file_path)
            return sent.id
        msg_id = cm.run_coroutine(_send_file())
        sess['completed'] = True
...
def api_upload_cancel():
    ...
    if session_id and session_id in upload_sessions:
        import shutil
        shutil.rmtree(upload_sessions[session_id]['temp_dir'], ignore_errors=True)
        del upload_sessions[session_id]

    return jsonify({'success': True, 'message': 'تم إلغاء الرفع'})


@app.route('/api/upload/resume', methods=['POST'])
def api_upload_resume():
    """استئناف الرفع — إرجاع آخر موضع تم رفعه"""
    data = request.get_json(force=True, silent=True) or {}
    session_id = data.get('session_id')
    ...
    sess = upload_sessions[session_id]
    uploaded = os.path.getsize(sess['file_path'])
    return jsonify({
        'success': True,
        'uploaded': uploaded,
        'total': sess['size'],
        'progress': int((uploaded / sess['size']) * 100) if sess['size'] > 0 else 0
    })


# ============================================================
#  الحزمة 2: معرض الوسائط + التمرير اللانهائي
# ============================================================
@app.route('/api/chat/<chat_id>/media', methods=['GET'])
def api_chat_media(chat_id):
    user_id = session.get('user_id')
    if not user_id:
        return jsonify({'success': False, 'message': 'غير مصرح'})
    ...
    try:
        from telethon.tl.types import (
            InputMessagesFilterPhotos, InputMessagesFilterVideo,
            InputMessagesFilterDocument, InputMessagesFilterVoice,
        ...
        async def _get_media():
            ...
            msgs = await cm.client.get_messages(
                entity,
                limit=limit,
                offset_id=offset_id,
                filter=filter_map.get(media_type, InputMessagesFilterEmpty())
            )
            result = []
            for m in msgs:
                if not m.media:
                ...
                if hasattr(m.media, 'photo'):
                    ...
                    media_info['thumb'] = f"/api/media/{chat_id}/{m.id}/thumb"
                    media_info['url'] = f"/api/media/{chat_id}/{m.id}"
                    ...
                    attrs = getattr(m.media.document, 'attributes', [])
                    fname = ''
                    for a in attrs:
                        fname = getattr(a, 'file_name', '')
                        if fname:
                            break
                    media_info['name'] = fname or 'ملف'
                    media_info['url'] = f"/api/media/{chat_id}/{m.id}"
                elif hasattr(m.media, 'voice') or getattr(m.media, 'voice', False):
        ...
        items = cm.run_coroutine(_get_media())
        return jsonify({
            'success': True,
            'media': items,
        ...
        logger.error(f"Chat media error: {e}")
        return jsonify({'success': False, 'message': str(e)})


@app.route('/api/media/<chat_id>/<msg_id>/thumb', methods=['GET'])
def api_media_thumb(chat_id, msg_id):
    """صورة مصغرة للوسائط"""
    user_id = session.get('user_id')
    if not user_id:
        return abort(401)
    ...
    try:
        async def _thumb():
            msgs = await cm.client.get_messages(int(chat_id), ids=int(msg_id))
            if not msgs or not msgs.media:
                return None
            return await msgs.download_media(file=bytes, thumb=-1)
        thumb = cm.run_coroutine(_thumb())
        if not thumb:
            return abort(404)
...
# ============================================================
#  الحزمة 3: المزامنة بين الأجهزة
# ============================================================
sync_subscriptions = {}  # {user_id: [session_ids]}
# {user_id: {sid: {id, name, type, last_active, is_online}}}
connected_devices   = {}


@socketio.on('sync_subscribe')
def handle_sync_subscribe(data):
    ...
    device_id   = (data or {}).get('device_id') or sid
    device_name = (data or {}).get('device_name') or 'جهاز غير معروف'
    device_type = (data or {}).get('device_type') or 'desktop'
    now_str     = datetime.now().strftime('%Y-%m-%d %H:%M')

    if user_id not in sync_subscriptions:
        sync_subscriptions[user_id] = []
    if user_id not in connected_devices:
        connected_devices[user_id] = {}

    if sid not in sync_subscriptions[user_id]:
        sync_subscriptions[user_id].append(sid)

    connected_devices[user_id][sid] = {
        'id':          device_id,
        'sid':         sid,
        'name':        device_name,
        'type':        device_type,
    ...
    emit('sync_ack', {'status': 'subscribed', 'device_id': device_id})

    # إخطار بقية أجهزة المستخدم بالجهاز الجديد
    for other_sid in sync_subscriptions.get(user_id, []):
        if other_sid != sid:
...
def _remove_device_session(user_id, sid):
    """إزالة جلسة الجهاز وإخطار بقية الأجهزة"""
    dev_info = None
    if user_id in connected_devices and sid in connected_devices[user_id]:
        dev_info = connected_devices[user_id].pop(sid)
    if user_id in sync_subscriptions:
        sync_subscriptions[user_id] = [s for s in sync_subscriptions[user_id] if s != sid]
    if dev_info:
        for other_sid in sync_subscriptions.get(user_id, []):
...
def broadcast_sync_update(user_id, data):
    """بث تحديث للمستخدم على جميع أجهزته"""
    if user_id in sync_subscriptions:
        for sid in sync_subscriptions[user_id]:
            socketio.emit('sync_update', data, to=sid)


def api_sync_status_info(user_id):
    """دالة مساعدة لحالة المزامنة — تستخدمها /api/sync/status الأصلية"""
    _settings = load_settings(user_id)
    return {
        'success': True,
        'last_sync': _settings.get('last_sync', None),
        'devices': len(sync_subscriptions.get(user_id, [])),
        'online': len(sync_subscriptions.get(user_id, [])) > 0
    }
...
def api_sync_force():
    ...
    _settings = load_settings(user_id)
    _settings['last_sync'] = datetime.now().isoformat()
    save_settings(user_id, _settings)

    broadcast_sync_update(user_id, {'type': 'force_sync', 'timestamp': datetime.now().isoformat()})
    return jsonify({'success': True, 'message': 'تمت المزامنة القسرية'})


@app.route('/api/sync/devices', methods=['GET'])
def api_sync_devices():
    """قائمة الأجهزة المتصلة حالياً بنفس الحساب"""
    user_id = session.get('user_id')
    if not user_id:
        return jsonify({'success': False}), 401

    my_sid   = request.headers.get('X-Socket-Id', '')
    devices  = []
    user_devs = connected_devices.get(user_id, {})

    for sid, info in user_devs.items():
        devices.append({
            'id':          info['id'],
            'sid':         sid,
            'name':        info.get('name', 'جهاز'),
            'type':        info.get('type', 'desktop'),
            'last_active': info.get('last_active', ''),
            'is_online':   info.get('is_online', True),
            'is_current':  (info['id'] == my_sid),
        })

    return jsonify({'success': True, 'devices': devices})


@app.route('/api/sync/device/<device_sid>', methods=['DELETE'])
def api_sync_terminate_device(device_sid):
    """إنهاء جلسة جهاز محدد"""
    user_id = session.get('user_id')
    if not user_id:
        return jsonify({'success': False}), 401

    if user_id in connected_devices and device_sid in connected_devices[user_id]:
        _remove_device_session(user_id, device_sid)
        # أبلغ الجهاز المُنهى بأنه طُرد
        socketio.emit('session_terminated', {'reason': 'terminated_by_another_device'}, to=device_sid)
        return jsonify({'success': True})
...
# ============================================================
#  الحزمة 4: إشارات المكالمات الصوتية/المرئية (WebRTC)
# ============================================================
call_rooms = {}  # {call_id: {...}}
...
def handle_call_initiate(data):
    ...
    if not chat_id or not user_id:
        emit('call_error', {'message': 'بيانات غير مكتملة'})
        return

    call_id = f"call_{chat_id}_{int(time.time())}"
    call_rooms[call_id] = {
        'offerer': request.sid,
        'answerer': None,
        'chat_id': chat_id,
...
def handle_call_accept(data):
    ...
    if call_id in call_rooms:
        call_rooms[call_id]['answerer'] = request.sid
        emit('call_accepted', {'call_id': call_id}, to=call_rooms[call_id]['offerer'])
...
def handle_call_reject(data):
    call_id = data.get('call_id')
    if call_id in call_rooms:
        emit('call_rejected', {'call_id': call_id}, to=call_rooms[call_id]['offerer'])
...
def handle_call_offer(data):
    call_id = data.get('call_id')
    sdp = data.get('sdp')
    if call_id in call_rooms and call_rooms[call_id]['answerer']:
        emit('call_offer_received', {'sdp': sdp}, to=call_rooms[call_id]['answerer'])
...
def handle_call_answer(data):
    call_id = data.get('call_id')
    sdp = data.get('sdp')
    if call_id in call_rooms and call_rooms[call_id]['offerer']:
        emit('call_answer_received', {'sdp': sdp}, to=call_rooms[call_id]['offerer'])
...
def handle_call_ice(data):
    ...
    if call_id in call_rooms:
        to_sid = call_rooms[call_id]['offerer'] if target == 'offerer' else call_rooms[call_id]['answerer']
        if to_sid:
            emit('call_ice_candidate_received', {'candidate': candidate}, to=to_sid)
...
def handle_call_end(data):
    call_id = data.get('call_id')
    if call_id in call_rooms:
        if call_rooms[call_id].get('answerer'):
            emit('call_ended', {'call_id': call_id}, to=call_rooms[call_id]['answerer'])
        if call_rooms[call_id].get('offerer'):
            emit('call_ended', {'call_id': call_id}, to=call_rooms[call_id]['offerer'])
        del call_rooms[call_id]


# ============================================================
#  الحزمة 5: الميزات الصغرى — مؤشر الكتابة
# ============================================================
@socketio.on('typing')
...
def handle_send_message(data):
    """إرسال رسالة عبر Telegram مع دعم الرد (reply_to)"""
    user_id = session.get('user_id')
    if not user_id:
        emit('message_error', {'error': 'غير مصرح'})
        return

    chat_id_raw = str(data.get('chat_id', '')).strip()
    text        = (data.get('text', '') or '').strip()
    reply_to    = data.get('reply_to_msg_id', None)
    tmp_id      = data.get('tmp_id')

    if not chat_id_raw or not text:
        emit('message_error', {'error': 'بيانات ناقصة', 'tmp_id': tmp_id})
        return
    ...
    if not client_mgr or not getattr(client_mgr, 'client', None):
        emit('message_error', {'error': 'العميل غير متصل', 'tmp_id': tmp_id})
        return
    ...
    try:
        async def _send():
            ...
            if reply_to:
                kwargs['reply_to'] = int(reply_to)
            sent = await client_mgr.client.send_message(entity, text, **kwargs)
            return sent.id

        real_id = client_mgr.run_coroutine(_send())
        emit('message_sent', {
            'tmp_id':   tmp_id,
            'msg_id':   real_id,
            'chat_id':  chat_id_raw,
        ...
        logger.info(f"[send_message] user={user_id} chat={chat_id_raw} reply={reply_to} → id={real_id}")
    except Exception as e:
        logger.error(f"[send_message] error: {e}")
        emit('message_error', {'error': str(e), 'tmp_id': tmp_id})


# ── الميزات الصغرى: المستخدمون المحظورون ─────────────────────────
...
def api_blocked_users():
    ...
    if not user_id:
        return jsonify({'success': False})
    _settings = load_settings(user_id)
    return jsonify({'success': True, 'blocked': _settings.get('blocked_users', [])})
...
def api_unblock_user():
    ...
    if not user_id or not target:
        return jsonify({'success': False})
    _settings = load_settings(user_id)
    blocked = _settings.get('blocked_users', [])
    if target in blocked:
        blocked.remove(target)
        _settings['blocked_users'] = blocked
        save_settings(user_id, _settings)
    return jsonify({'success': True, 'message': 'تم رفع الحظر'})

# ══════════════════════════════════════════════════════════════
#  نهاية الإضافات — الحزم 1-5
# ══════════════════════════════════════════════════════════════


# ============================================================
#  مسارات إضافية — دعم واجهة تليجرام الجديدة
# ============================================================

# ---- /api/chats — قائمة المحادثات (تغليف /api/get_conversations) ----
@app.route("/api/chats", methods=["GET"])
def api_chats_list():
    """قائمة المحادثات بتنسيق متوافق مع الواجهة الجديدة."""
    if 'user_id' not in session:
        return jsonify({"success": False, "message": "غير مصرح"}), 401
    user_id = session['user_id']
    # إعادة استخدام منطق api_get_conversations
    try:
        resp = api_get_conversations()
        ...
        if data.get('success'):
            ...
            # تحويل التنسيق ليتوافق مع الواجهة
            chats = []
            for d in dialogs:
                last_ts = 0
                try:
                    if d.get('last_time'):
                        ...
                        last_ts = int(datetime.datetime(now.year, now.month, now.day, h, m).timestamp())
                except Exception:
                    pass
                chats.append({
                    "id": str(d.get('id', '')),
                    "name": d.get('name', d.get('title', 'محادثة')),
                    "title": d.get('name', d.get('title', 'محادثة')),
                    "is_group": d.get('type') == 'group' or d.get('is_group', False),
                    "is_channel": d.get('type') == 'channel' or d.get('is_channel', False),
                    "unread_count": d.get('unread', d.get('unread_count', 0)),
                    "last_message": {"text": d.get('last_message', ''), "timestamp": last_ts},
                    "last_ts": last_ts,
                    "pinned": d.get('pinned', False),
                    "archived": d.get('archived', False),
                })
            return jsonify({"success": True, "chats": chats, "total": len(chats)})
    except Exception as e:
        logger.error(f"api_chats_list error: {e}")
    return jsonify({"success": False, "message": "خطأ في تحميل المحادثات"}), 500


# ---- /api/chats/<chat_id>/messages — رسائل محادثة ----
@app.route("/api/chats/<chat_id>/messages", methods=["GET"])
def api_chats_messages(chat_id):
    """رسائل محادثة معينة (تغليف /api/get_messages)."""
    if 'user_id' not in session:
        return jsonify({"success": False, "message": "غير مصرح"}), 401
            ...
            async def _msgs():
                try:
                    ...
                    try:
                        ...
                        return []
                msgs = await cm.client.get_messages(
                    entity, limit=min(int(limit), 100),
                    offset_id=int(offset_id), reverse=False
                )
                result = []
                for m in msgs:
                    if not m:
                        continue
                    fn = (getattr(m.sender, 'first_name', '') or '') if m.sender else ''
                    ln = (getattr(m.sender, 'last_name',  '') or '') if m.sender else ''
                    sender_name = (fn + ' ' + ln).strip() or (getattr(m.sender, 'username', '') or '') if m.sender else 'مستخدم'
                    result.append({
                        'id': m.id,
                        'text': m.text or '',
                        'date': int(m.date.timestamp()) if m.date else 0,
                        'timestamp': int(m.date.timestamp()) if m.date else 0,
                        'out': m.out,
                        'from_me': m.out,
                        'sender_id': getattr(m.sender_id, 'user_id', m.sender_id) if m.sender_id else None,
                        'sender_name': sender_name,
                        'edited': bool(getattr(m, 'edit_date', None)),
                        'media': bool(m.media),
                    })
                return result
            messages = cm.run_coroutine(_msgs()) or []
            return jsonify({"success": True, "messages": messages, "count": len(messages)})
    except Exception as e:
        logger.error(f"api_chats_messages error: {e}")
    return jsonify({"success": False, "messages": [], "message": str(e) if 'e' in dir() else "خطأ"})


# ---- /api/chats/<chat_id>/pin — تثبيت المحادثة ----
@app.route("/api/chats/<chat_id>/pin", methods=["POST"])
def api_chat_pin(chat_id):
    if 'user_id' not in session:
        return jsonify({"success": False}), 401
    user_id = session['user_id']
    data = request.get_json(force=True, silent=True) or {}
    ...
    try:
        ...
        s = {}
    plist = s.get('pinned', [])
    if pinned and str(chat_id) not in [str(x) for x in plist]:
        plist.append(str(chat_id))
    elif not pinned:
        plist = [x for x in plist if str(x) != str(chat_id)]
    s['pinned'] = plist
    try:
        os.makedirs('data', exist_ok=True)
        with open(path, 'w', encoding='utf-8') as f:
...
# ---- DELETE /api/chats/<chat_id> — حذف/مغادرة محادثة ----
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _delete():
                ...
                if hasattr(entity, 'megagroup') or hasattr(entity, 'broadcast'):
                    ...
                    await cm.client(LeaveChannelRequest(channel=entity))
                else:
                    from telethon.tl.functions.messages import DeleteHistoryRequest
                    await cm.client(DeleteHistoryRequest(peer=entity, max_id=0, just_clear=False, revoke=False))
                return True
            cm.run_coroutine(_delete())
    except Exception as e:
        logger.error(f"api_chat_delete error: {e}")
        return jsonify({"success": False, "message": str(e)})
    return jsonify({"success": True})


# ---- /api/messages/send — إرسال ملف (alias لـ send-media) ----
@app.route("/api/messages/send", methods=["POST"])
def api_messages_send_file():
    """إرسال ملف — يعيد التوجيه إلى api_send_media."""
    return api_send_media()


# ---- /api/auth/logout — تسجيل الخروج (alias) ----
@app.route("/api/auth/logout", methods=["POST"])
def api_auth_logout_alias():
    return api_user_logout()


# ---- /api/user/info — معلومات المستخدم (alias لـ /api/me) ----
@app.route("/api/user/info", methods=["GET"])
def api_user_info():
    """معلومات المستخدم الحالي مع تنسيق موحد."""
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'غير مصرح'}), 401
    user_id = session['user_id']
    with USERS_LOCK:
        client_mgr = USERS.get(user_id, {}).get('client_manager')
    if not client_mgr or not getattr(client_mgr, 'client', None):
        # إرجاع بيانات من PREDEFINED_USERS
        u = PREDEFINED_USERS.get(user_id, {})
        return jsonify({'success': True, 'user': {
            'id': user_id,
            'name': u.get('name', 'مستخدم'),
            'first_name': u.get('name', 'مستخدم'),
            'last_name': '',
            'phone': '',
            'username': '',
        }})
    try:
        ...
        logger.error(f'api_user_info error: {e}')
        return jsonify({'success': False, 'message': str(e)})


# ============================================================
#  مسارات صفحات الشريط الجانبي (Drawer pages)
# ============================================================
...
# ─── البحث عن مستخدمين (للمحادثة الجديدة) ──────────────────────────────────
@app.route("/api/search/users", methods=["GET"])
def api_search_users():
    """البحث عن مستخدمين بالاسم أو اسم المستخدم."""
    if 'user_id' not in session:
        return jsonify({"success": False}), 401
    q       = request.args.get('q', '').strip()
    limit   = int(request.args.get('limit', 20))
    ...
    try:
        ...
        if cm and hasattr(cm, 'client') and cm.client:
            async def _search():
                from telethon.tl.functions.contacts import SearchRequest
                r = await cm.client(SearchRequest(q=q, limit=limit))
                out = []
                for u in getattr(r, 'users', []):
                    ...
                    out.append({
                        'id': str(u.id),
                        'name': (fn + ' ' + ln).strip() or getattr(u, 'username', '') or str(u.id),
    ...
    return jsonify({"success": True, "users": results[:limit]})
...
def api_add_story():
    ...
    if not media_file and not caption:
        return jsonify({'success': False, 'message': 'أضف صورة أو نصاً'})
    try:
        tmp_path = None
        if media_file:
            import tempfile as _tmp
            suffix = os.path.splitext(media_file.filename)[1] or '.jpg'
            with _tmp.NamedTemporaryFile(delete=False, suffix=suffix) as f:
                media_file.save(f.name)
                tmp_path = f.name
        async def _post_story():
            try:
                from telethon.tl.functions.stories import SendStoryRequest
                from telethon.tl.types import InputPrivacyValueAllowAll, MediaArea
                privacy = [InputPrivacyValueAllowAll()]
                if tmp_path:
                    uploaded = await client_mgr.client.upload_file(tmp_path)
                    from telethon.tl.types import InputMediaUploadedPhoto
                    media = InputMediaUploadedPhoto(file=uploaded)
                    await client_mgr.client(SendStoryRequest(
                        peer='me',
                        media=media,
                        caption=caption or '',
                    ...
                    # قصة نصية فقط — إرسال رسالة محفوظة بديلاً
                    await client_mgr.client.send_message('me', f'📖 قصة: {caption}')
                return True
            except Exception as _e:
                logger.warning(f"SendStoryRequest failed ({_e}), fallback to saved message")
                if caption:
                    await client_mgr.client.send_message('me', f'📖 قصة: {caption}')
                return True
        client_mgr.run_coroutine(_post_story())
        if tmp_path:
            try: os.unlink(tmp_path)
            except: pass
        return jsonify({'success': True, 'message': 'تم نشر القصة بنجاح'})
    except Exception as e:
        logger.error(f'api_add_story error: {e}')
        return jsonify({'success': False, 'message': str(e)})

# ============================================================
#  نهاية مسارات واجهة تليجرام الجديدة
# ============================================================
